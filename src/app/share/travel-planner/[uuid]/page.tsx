...
export default function SharedTravelPlanPage() {
    ...
    const fetchSharedData = async () => {
        try {
            setLoading(true)
            const response = await fetch(`/api/get-shared-data/${uuid}`)
            const result = await response.json()

            if (result.status && result.data) {
                setSharedData(result.data)
            } else {
                setError(result.message || "Failed to load shared data")
            }
        } catch (err) {
            console.error("Error fetching shared data:", err)
            setError("Failed to load shared travel plan")
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const dataToExport = sharedData.agent_outputs
            const destinationName = getUserInputValue("Destination") || "Travel Plan"
            const filename = `Travel-Plan-${destinationName}.pdf`
    ...
    const getUserInputValue = (key: string) => {
        if (!sharedData) return null
        const rawValue = sharedData.user_inputs.find((item: any) => item.variable === key)?.variable_value

        if (!rawValue || rawValue === "None") return null

        // Try to parse arrays like "['relaxed', 'romantic']"
    ...
    const renderUserConfiguration = () => {
        if (!sharedData?.user_inputs?.length) return null

        const configGroups = [
            {
                title: "Trip Details",
                inputs: ["Travelling_From", "Destination", "Start_Date", "End_Date"],
            },
            {
                title: "Travelers",
                inputs: ["Number_of_Adults", "Number_of_Children", "Children_Ages"],
            },
            {
                title: "Preferences",
                inputs: ["Budget", "Preferred_Transportation", "Preferred_Local_Transportation", "Preferred_Accommodation"],
            },
            {
                title: "Additional Info",
                inputs: ["Dietary_Restrictions", "Special_Requests", "Traveling_Style", "Interests"],
            },
        ]
                ...
                >
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-base font-semibold text-gray-900">Trip Configuration</h3>
                        </div>
                        {showUserConfig ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {configGroups.map((group, groupIndex) => (
                                <div key={groupIndex} className="space-y-3">
                                    <h4 className="text-sm font-semibold text-gray-800 border-b border-gray-200 pb-1">{group.title}</h4>
                                    <div className="space-y-2">
                                        {group.inputs.map((inputKey) => {
                                            const value = getUserInputValue(inputKey)
                                            if (!value) return null

                                            const label = inputKey.replace(/_/g, " ").replace(/\b\w/g, (l) => l.toUpperCase())

                                            return (
                                                <div key={inputKey} className="flex justify-between items-center py-1">
                                                    <span className="text-xs text-gray-600 font-medium">{label}:</span>
                                                    <span className="text-xs text-gray-900 font-semibold max-w-[200px] text-right">
    ...
    // Helper function to render itinerary
            ...
            <div key={index} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm min-w-0">
                ...
                <div className="p-3 sm:p-4 space-y-3">
                    {day.activities?.map((activity: any, actIndex: number) => (
                        <div key={actIndex}>
                            {actIndex > 0 && <div className="border-t border-gray-200 my-4"></div>}

                            <div className="p-3 hover:shadow-md transition-shadow min-w-0">
    ...
    // Helper function to render hotels
    ...
    // Helper function to render restaurants
    ...
    // Helper function to render transportation
    ...
    // Helper function to render places
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center max-w-md mx-auto p-6">
                    ...
                    <h1 className="text-xl font-semibold text-gray-900 mb-2">Travel Plan Not Found</h1>
    ...
    if (!sharedData) {
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    <p className="text-gray-600">No travel plan data available</p>
                </div>
            </div>
        )
    }

    const dataToDisplay = sharedData.agent_outputs
    const destinationName = getUserInputValue("Destination")
                            ...
                            <div className="min-w-0 flex-1">
                                <h1 className="text-lg sm:text-xl font-bold flex flex-col sm:flex-row sm:items-center gap-2">
                                    <span className="break-words">
                                        {destinationName ? `Perfect Trip to ${destinationName}` : "Travel Plan"}
                                    </span>
                                    {destinationName && (
                                        <Button
                                            size="sm"
                ...
                <div className="mt-6 sm:mt-8 text-center">
                    <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">
                            Want Your Own Personalized Travel Plan?
                        </h3>
                        <p className="text-sm text-gray-600 mb-4">
                            Create custom travel plans tailored to your preferences, destination, and travel dates.
                        </p>
                        <Link href="/?scrollTo=agents">
