...
import { HistoryIcon, Home, BookOpen } from "lucide-react"
import DynamicAgentForm from "./LessonForm"
import { Button } from "../ui/button"
import FormattedResponse from "./LessonResponse"
import HistorySidebar from "./LessonHistory"
import MobileHistorySidebar from "./MobileHistory"
...
export default function LessonPlannerScreen() {
    ...
    const [currentChapterOrTopicName, setCurrentChapterOrTopicName] = useState<string | null>(null)
    ...
    const handleHistoryItemClick = async (execution_id: string, agent_id: string) => {
        try {
            ...
            setCurrentChapterOrTopicName(null)
            ...
            // Extract chapter/topic name from historical data
            let chapterOrTopicName = null
            if (data.user_inputs) {
                const topicInput = data.user_inputs.find((input: any) => input.variable === "Topic")
                const chapterNameInput = data.user_inputs.find((input: any) => input.variable === "Chapter_Name")

                if (topicInput) {
                    chapterOrTopicName = topicInput.variable_value
                } else if (chapterNameInput) {
                    chapterOrTopicName = chapterNameInput.variable_value
                }
            }
            setCurrentChapterOrTopicName(chapterOrTopicName)

            // Process historical inputs
            let inputsToLoad: any = null
            if (data.user_inputs) {
            ...
            if (inputsToLoad && formRef.current?.loadHistoryData) {
                ...
                // Extract the values we need for cascade loading
                const boardValue = inputsToLoad.find((input: any) => input.variable === "Board")?.variable_value
                const gradeValue = inputsToLoad.find((input: any) => input.variable === "Grade")?.variable_value
                const subjectValue = inputsToLoad.find((input: any) => input.variable === "Subject")?.variable_value

                // Load historical data into form first
                formRef.current.loadHistoryData(inputsToLoad as any, data.file_data)

                // Then trigger cascade loading with proper sequencing and delays
                if (boardValue) {
                    // Wait a bit for the form to process the initial data
                    setTimeout(async () => {
                        try {
                            // Fetch grades for the historical board
                            await fetch(`/api/dropdown-values?board=${encodeURIComponent(boardValue)}`)
                                .then((res) => res.json())
                                .then(async (gradeData) => {
                                    if (gradeData.status && gradeValue) {
                                        // Wait a bit more, then fetch subjects
                                        setTimeout(async () => {
                                            try {
                                                await fetch(
                                                    `/api/dropdown-values?board=${encodeURIComponent(boardValue)}&grade=${encodeURIComponent(gradeValue)}`,
                                                )
                                                    .then((res) => res.json())
                                                    .then(async (subjectData) => {
                                                        if (subjectData.status && subjectValue) {
                                                            // Wait a bit more, then fetch chapters
                                                            setTimeout(async () => {
                                                                try {
                                                                    await fetch(
                                                                        `/api/dropdown-values?board=${encodeURIComponent(boardValue)}&grade=${encodeURIComponent(gradeValue)}&subject=${encodeURIComponent(subjectValue)}`,
                                                                    )
                                                                        .then((res) => res.json())
                                                                        .then(() => {
                                                                            // Final reload of historical data after all dropdowns are populated
                                                                            setTimeout(() => {
                                                                    ...
                                                                    console.warn("Error fetching chapters for history:", error)
                                                                }
                                                            }, 300)
                                                        }
                                                    })
                                            } catch (error) {
                                                console.warn("Error fetching subjects for history:", error)
                                            }
                                        }, 300)
                                    }
                                })
                        } catch (error) {
                            console.warn("Error fetching grades for history:", error)
                        }
                    }, 200)
                } else {
                    // If no cascade loading needed, just force update after a short delay
                    setTimeout(() => {
                        if (formRef.current?.forceUpdate) {
                            formRef.current.forceUpdate()
                        }
            ...
            if (data.file_data && data.file_data.length > 0) {
                ...
                setFileName(chapterOrTopicName || "historical_response")
            }
    ...
    const handleFormResponse = (res: any) => {
        ...
        if (res.loading) {
            ...
            // Set chapter/topic name from form response
            if (res.chapterOrTopicName) {
                setCurrentChapterOrTopicName(res.chapterOrTopicName)
                setFileName(res.chapterOrTopicName)
            }
    ...
    if (loading || showLoader) {
        return (
            <main className="flex flex-col items-center justify-center h-screen w-screen">
                <h2 className="text-xl font-semibold text-gray-700 mb-6">Lesson Planner</h2>
                <LessonPlannerLoader />
        ...
        <main className="relative min-h-screen bg-gray-50">
            ...
            <button
                ...
                className={`fixed left-4 bottom-6 z-40 md:hidden
                    bg-gradient-to-r from-blue-600 to-purple-600 text-white p-3 sm:p-4 rounded-full shadow-xl
                    hover:from-blue-700 hover:to-purple-700 transition-all duration-300 ease-out
