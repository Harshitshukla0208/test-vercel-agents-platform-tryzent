...
import DynamicTravelForm from "./TravelForm"
import { Button } from "../ui/button"
import FormattedTravelResponse from "./TravelPlannerResponse"
import TravelHistorySidebar from "./TravelPlannerHistory"
import MobileTravelHistorySidebar from "./MobileHistory"
...
import { TravelPlannerLoader } from "./TravelPlannerLoader"
...
export default function TravelPlannerScreen() {
    ...
    const [currentDestinationName, setCurrentDestinationName] = useState<string | null>(null)
    ...
    const handleHistoryItemClick = async (execution_id: string, agent_id: string) => {
        try {
            ...
            setCurrentDestinationName(null)
            ...
            // Extract destination name from historical data
            let destinationName = null
            if (data.user_inputs) {
                const destinationInput = data.user_inputs.find((input: any) => input.variable === "Destination")
                if (destinationInput) {
                    destinationName = destinationInput.variable_value
                }
            }
            ...
            if (data.file_data && data.file_data.length > 0) {
                ...
                setFileName(destinationName || "historical_response")
            }
    ...
    const handleFormResponse = (res: any) => {
        ...
        if (res.loading) {
            ...
            // Set destination name from form response
            if (res.destinationName) {
                setCurrentDestinationName(res.destinationName)
                setFileName(res.destinationName)
            }
    ...
    if (loading || showLoader) {
        return (
            <main className="flex flex-col items-center justify-center h-screen w-screen">
                <h2 className="text-xl font-semibold text-gray-700 mb-6">Travel Planner</h2>
                <TravelPlannerLoader />
