...
import { RefreshCcw, Clock, Shield, ChevronDown, Plus, User, DollarSign, Users, Building2, Heart } from 'lucide-react';
...
interface MobileHealthInsuranceHistoryProps {
...
const MobileHealthInsuranceHistory: React.FC<MobileHealthInsuranceHistoryProps> = ({
    onHistoryItemClick,
    containerRef,
    formSectionRef,
    refreshTrigger,
    ...
    // Height calculation effect for mobile
    useEffect(() => {
        const updateHeight = () => {
            let totalHeight = 0;
            ...
            if (hasContent) {
                // For mobile, use a more conservative height calculation
                const viewportHeight = window.innerHeight;
                const minHeight = viewportHeight * 0.7; // 70vh minimum for mobile
                const finalHeight = Math.max(totalHeight + 150, minHeight); // Add 150px padding for better spacing
                setMaxHeight(`${finalHeight}px`);
            } else {
                // Fallback to viewport-based height for mobile
                const viewportHeight = window.innerHeight;
    ...
    const getPrimaryFields = (userInputs: VariableInput[]): VariableInput[] => {
        ...
        ).slice(0, 2); // Limit to 2 primary fields for mobile
    };
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="pt-6 px-4 pb-4 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-indigo-50">
            ...
            {onCreateNew && (
                <div className="p-3 border-b border-gray-100 bg-white">
                    <button
                        type="button"
                        onClick={onCreateNew}
                        className="w-full h-10 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md transition-colors duration-200 shadow-sm flex items-center justify-center gap-2"
                    >
                                    ...
                                    <div key={threadId} className="space-y-2">
                                        ...
                                        <div
                                            className={`group relative bg-white rounded-xl border-2 cursor-pointer transition-all duration-200 hover:shadow-md ${
                                                isSelected
                                                    ? 'border-[#3b82f6] bg-indigo-50 shadow-lg'
                                        ...
                                        >
                                            <div className="p-4">
                                                ...
                                                {/* Primary Fields (Limited for mobile) */}
...
export default MobileHealthInsuranceHistory;