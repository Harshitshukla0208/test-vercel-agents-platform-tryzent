...
interface HealthInsuranceFinderLoaderProps {
    isVisible?: boolean;
}

export default function HealthInsuranceFinderLoader({ isVisible = true }: HealthInsuranceFinderLoaderProps) {
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 mx-3 sm:mx-4 max-w-xs sm:max-w-sm md:max-w-md w-full animate-fade-in">
                <div className="flex flex-col items-center text-center space-y-4 sm:space-y-6">
                    ...
                    <div className="relative w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 flex items-center justify-center">
                        ...
                        >
                            <Shield className="w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 text-indigo-600" />
                        </motion.div>
                    </div>
                    ...
                    <div className="space-y-2 sm:space-y-3">
                        ...
                        <p className="text-xs sm:text-sm text-gray-600 opacity-80 leading-relaxed px-1 sm:px-2">
                            We're analyzing health insurance options tailored just for you! 🏥🛡️
                        </p>
                    </div>
                </div>
            </div>
...
// Response Loader Component
interface HealthInsuranceResponseLoaderProps {
    isVisible?: boolean;
}

export function HealthInsuranceResponseLoader({ isVisible = true }: HealthInsuranceResponseLoaderProps) {
    if (!isVisible) return null;
