...
import DynamicHealthInsuranceForm from "./HealthInsuranceForm"
import { Button } from "../ui/button"
import FormattedHealthInsuranceResponse from "./HealthInsuranceResponse"
import HealthInsuranceHistorySidebar from "./HealthInsuranceHistory"
import MobileHealthInsuranceHistorySidebar from "./MobileHistory"
...
import HealthInsuranceLoader from "./HealthInsuranceLoader"
...
export default function HealthInsuranceFinderScreen() {
    ...
    const [currentInsuranceType, setCurrentInsuranceType] = useState<string | null>(null)
    ...
    const handleHistoryItemClick = async (execution_id: string, agent_id: string) => {
        try {
            ...
            // Extract insurance type from historical data
            let insuranceType = null
            if (data.user_inputs) {
                const insuranceInput = data.user_inputs.find((input: any) => input.variable === "InsuranceType" || input.variable === "insurance_type")
                if (insuranceInput) {
                    insuranceType = insuranceInput.variable_value
                }
            }
            setCurrentInsuranceType(insuranceType)
            ...
            if (inputsToLoad && formRef.current?.loadHistoryData) {
                ...
                formRef.current.loadHistoryData(inputsToLoad)
            ...
            if (data.file_data && data.file_data.length > 0) {
                ...
                setFileName(insuranceType || "historical_response")
            }
    ...
    const handleFormResponse = (res: any) => {
        ...
        if (res.loading) {
            ...
            // Set insurance type from form response
            if (res.insuranceType) {
                setCurrentInsuranceType(res.insuranceType)
                setFileName(res.insuranceType)
            }
    ...
    if (loading || showLoader) {
        return (
            <main className="flex flex-col items-center justify-center h-screen w-screen">
                <h2 className="text-xl font-semibold text-black mb-6">Health Insurance Finder</h2>
                <HealthInsuranceLoader />
                <p className="text-black text-sm mt-4 text-center">
                    Finding Your Perfect Insurance Plan...
                </p>
            </main>
        )
    }
    ...
    if (error) {
        return (
            <main className="relative min-h-screen bg-indigo-50">
                <div className="flex items-center justify-center h-screen p-4">
                    <div className="text-center">
                        <div className="text-red-600 text-lg sm:text-xl mb-4">Error</div>
                        <p className="text-sm sm:text-base text-indigo-600">{error}</p>
                    </div>
                </div>
            </main>
        )
        ...
        <main className="relative min-h-screen bg-indigo-50">
                ...
                >
                    ...
                    >
                        <div className="h-full pt-2">
                            <MobileHealthInsuranceHistorySidebar
            ...
            <button
                ...
                className={`fixed left-4 bottom-6 z-40 md:hidden
                    bg-gradient-to-r from-indigo-600 to-indigo-800 text-white p-3 sm:p-4 rounded-full shadow-xl
                    hover:from-indigo-700 hover:to-indigo-900 transition-all duration-300 ease-out
            ...
            {/* Header */}
            <header className="bg-white border-b border-indigo-200 sticky top-0 z-10">
                ...
                <div className="max-w-7xl mx-auto p-3 sm:p-4">
                    <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                            {/* Form Section */}
                            <div ref={formSectionRef} className="bg-white rounded-xl shadow-sm border border-indigo-200 mb-4 sm:mb-6">
                                {agent && (
                                    <DynamicHealthInsuranceForm
                                        ref={formRef}
                                        agentData={agent}
                            ...
                            <div ref={formattedResponseRef} className="w-full">
                                {response && (
                                    <FormattedHealthInsuranceResponse
