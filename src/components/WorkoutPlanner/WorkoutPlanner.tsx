...
import DynamicWorkoutForm from "./WorkoutForm"
import { Button } from "../ui/button"
import FormattedWorkoutResponse from "./WorkoutPlannerResponse"
import WorkoutHistorySidebar from "./WorkoutPlannerHistory"
import MobileWorkoutHistorySidebar from "./MobileWorkoutHistory"
...
import { WorkoutPlannerLoader } from "./WorkoutPlannerLoader"
...
export default function WorkoutPlannerScreen() {
    ...
    const handleHistoryItemClick = async (execution_id: string, agent_id: string) => {
        try {
            ...
            // Extract workout type from historical data
            let workoutType = null
            if (data.user_inputs) {
                const userTypeInput = data.user_inputs.find((input: any) => input.variable === "user_type")
                if (userTypeInput) {
                    workoutType = userTypeInput.variable_value
                }
            }
            ...
            if (data.file_data && data.file_data.length > 0) {
                ...
                setFileName(workoutType || "historical_response")
            }
    ...
    const handleFormResponse = (res: any) => {
        ...
        if (res.loading) {
            ...
            // Set workout type from form response
            if (res.workoutType) {
                setCurrentWorkoutType(res.workoutType)
                setFileName(res.workoutType)
            }
    ...
    if (loading || showLoader) {
        return (
            <main className="flex flex-col items-center justify-center h-screen w-screen">
                <h2 className="text-xl font-semibold text-gray-700 mb-6">Workout Planner</h2>
                <WorkoutPlannerLoader />
                <p className="text-gray-500 text-sm mt-4 text-center">Creating Your Fitness Plan...</p>
            </main>
        )
    }
