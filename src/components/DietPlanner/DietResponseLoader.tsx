...
interface DietResponseLoaderProps {
  isVisible?: boolean
}

export default function DietResponseLoader({ isVisible = true }: DietResponseLoaderProps) {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 mx-3 sm:mx-4 max-w-xs sm:max-w-sm md:max-w-md w-full animate-fade-in">
        <div className="flex flex-col items-center text-center space-y-4 sm:space-y-6">
          ...
          <div className="relative w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 flex items-center justify-center">
            ...
            >
              <Utensils className="w-8 h-8 sm:w-12 sm:h-12 md:w-14 md:h-14 text-blue-600" />
            </motion.div>
          </div>
          ...
          <div className="space-y-2 sm:space-y-3">
            <h2 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800"> Creating Your Perfect Diet Plan...</h2>
            <p className="text-xs sm:text-sm text-gray-600 opacity-80 leading-relaxed px-1 sm:px-2">
              Balancing nutrients and your goals for a healthier you 🥗
            </p>
          </div>
        </div>
      </div>
