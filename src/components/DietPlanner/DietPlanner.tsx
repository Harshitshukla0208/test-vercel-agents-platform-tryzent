...
import DynamicDietForm from "./DietForm"
import { Button } from "../ui/button"
import FormattedDietResponse from "./DietPlannerResponse"
import DietHistorySidebar from "./DietPlannerHistory"
import MobileDietHistorySidebar from "./MobileDietHistory"
...
import { DietPlannerLoader } from "./DietPlannerLoader"
...
export default function DietPlannerScreen() {
  ...
  const [currentDietType, setCurrentDietType] = useState<string | null>(null)
  ...
  const handleHistoryItemClick = async (execution_id: string, agent_id: string) => {
    try {
      ...
      // Extract diet type from historical data
      let dietType = null
      if (data.user_inputs) {
        const dietGoalInput = data.user_inputs.find((input: any) => input.variable === "diet_goal")
        if (dietGoalInput) {
          dietType = dietGoalInput.variable_value
        }
      }
      setCurrentDietType(dietType)
      ...
      if (data.file_data && data.file_data.length > 0) {
        ...
        setFileName(dietType || "historical_response")
      }
  ...
  const handleFormResponse = (res: any) => {
    ...
    if (res.loading) {
      ...
      // Set diet type from form response
      if (res.dietType) {
        setCurrentDietType(res.dietType)
        setFileName(res.dietType)
      }
  ...
  if (loading || showLoader) {
    return (
      <main className="flex flex-col items-center justify-center h-screen w-screen">
        <h2 className="text-xl font-semibold text-gray-700 mb-6">Diet Planner</h2>
        <DietPlannerLoader />
        <p className="text-gray-500 text-sm mt-4 text-center">Creating Your Nutrition Plan...</p>
      </main>
    )
  }
    ...
    <main className="relative min-h-screen bg-gray-50">
      ...
      <button
        ...
        className={`fixed left-4 bottom-6 z-40 md:hidden
                    bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-3 sm:p-4 rounded-full shadow-xl
                    hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 ease-out
