...
import { RefreshCcw, ChevronDown, Clock, BookOpen, Plus } from 'lucide-react';
...
interface HistorySidebarProps {
    ...
    onCreateNew?: () => void;
    createNewClassName?: string; // optional class override per theme
}
...
const HistorySidebar: React.FC<HistorySidebarProps> = ({
    ...
    const getPrimaryDisplayValue = (userInputs: VariableInput[]) => {
        // Try to find a meaningful primary value to display
        const priorityFields = ['destination', 'subject', 'topic', 'diet_goal', 'user_type'];
        
        for (const field of priorityFields) {
            const input = userInputs.find(input => 
                input.variable.toLowerCase().includes(field)
            );
            if (input) return input.variable_value;
        }
        
        // Fallback to first non-empty value
        const firstInput = userInputs.find(input => input.variable_value && input.variable_value.trim());
        return firstInput ? firstInput.variable_value : 'New Entry';
    };
    ...
    return (
        <div
            ref={sidebarRef}
            className="w-56 relative bg-white border-r border-gray-200 rounded-md"
            style={{
                height: maxHeight,
                minHeight: '400px',
                overflow: 'auto'
        ...
        >
            ...
            {error && (
                <div className="text-red-500 text-xs text-center p-2 bg-red-50 rounded-md">
                    {error}
                </div>
            )}
